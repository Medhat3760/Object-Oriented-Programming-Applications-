...
using MyLib;
using static Bank_System.Core.clsUser;

namespace Bank_System.Core
{
    public class clsUser : clsPerson
    {
        ...
        private static clsUser _ConvertLineToUserObject(string line, string separator = "#//#")
        {
            ...
            return new clsUser(enMode.UpdateMode, lUserData[0], lUserData[1], lUserData[2], lUserData[3], lUserData[4], clsUtil.DecryptText(lUserData[5]), int.Parse(lUserData[6]));
        ...
        private string _PrepareLoginRecord(string separator = "#//#")
        {

            string loginRecord = clsDate.GetSystemDateTimeString() + separator
                + UserName + separator
                + clsUtil.EncryptText(Password) + separator
        ...
        private static stLoginRegisterRecord _ConvertLoginRegisterLineToRecord(string line, string separator = "#//#")
        {

            stLoginRegisterRecord loginRegisterRecord;

            List<string> listLoginRegisterData = clsString.Split(line, separator);


            loginRegisterRecord.dateTime = listLoginRegisterData[0];
            loginRegisterRecord.userName = listLoginRegisterData[1];
            loginRegisterRecord.password = clsUtil.DecryptText(listLoginRegisterData[2]);
            loginRegisterRecord.permissions = int.Parse(listLoginRegisterData[3]);
        ...
        public bool Delete()
        {
            ...
            for (int i = 0; i < lUsers.Count; i++)
            {
                ...
                if (lUsers[i].UserName == UserName)
                {
                    lUsers[i]._markedForDelete = true;
                    break;
                }
        ...
        public bool CheckAccessPermission(enPermissions permission)
        {
            ...
            return Permissions == (int)enPermissions.pAll || (Permissions & (int)permission) == (int)permission;
        ...
        public void RegisterLogin()
        {
            ...
            if (File.Exists("LoginRegister.txt"))
            {

                string dataLine;

                using (StreamWriter myFile = new StreamWriter("LoginRegister.txt", append: true))
                {
        ...
        public static List<stLoginRegisterRecord> GetLoginRegisterList()
        {
            ...
            if (File.Exists("LoginRegister.txt"))
            {
                ...
                using (StreamReader myFile = new StreamReader("LoginRegister.txt"))
                {
                    ...
                    while ((line = myFile.ReadLine()) != null)
                    {
                        ...
                        listLoginRegisterRecord.Add(loginRegisterRecord);
